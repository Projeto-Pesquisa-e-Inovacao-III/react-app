...
export default function Dashboard() {
    ...
    const quantitySessionInMonth = useQuery({
        queryKey: ['sessions'],
        queryFn: async () => {
    ...
    const activeStudentsCount = useQuery({
        ...
        queryFn: async () => {
            ...
            return result.data.quantidadeAlunos;
        }
    });
    ...
    const inactiveStudents = useQuery({
        ...
        queryFn: async () => {
            ...
            return {
                count: result.data.quantidadeAlunos,
                percentage: result.data.percentualAlunos
            };
        }
    });
            ...
            <div className={styles.wrapperDashboardPersonal}>
                <div className={classnames(styles.wrapperElementsDashboard, { [styles.dashMobile]: isMobile })}>
                    ...
                    <div className={classnames(styles.kpisDashboardPersonal, { [styles.kpisDashboardPersonalMobile]: isMobile })}>
                        ...
                        isMobile={isMobile} 
                        isFull={!isMobile} 
                        title="Alunos com pacotes ativos" 
                        value={activeStudentsCount.data} 
                        ...
                        isMobile={isMobile} 
                        isFull={!isMobile} 
                        title="Vendas dos ultimos 30 dias" 
                        value={plansSalesQuantity.data} 
                        description="Vendas dos ultimos 30 dias"
                    ...
                    <div className={styles.wrapperChartsDashboardPersonal}>
                        <Chart data={quantitySessionInMonth.data} isLoading={quantitySessionInMonth.isLoading} type="bar" title="Consultorias por mês" titleY="Quantidade de consultorias" titleX="Mês" legend="Consultorias" ooffsetY={1} />
                        <Chart data={sallesCount.data} isLoading={sallesCount.isLoading} type="line" title="Ganhos Mensais" titleY="Quantidade de vendas em R$" titleX="Mês" legend="Ganhos" ooffsetY={15} />
                    </div>
                </div>
            </div>
        </>
